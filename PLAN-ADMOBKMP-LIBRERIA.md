...
> **14 sep 2026:** el tag `1.0.0` se publicó y **falló en JitPack** por `iosX64` (ver §6), y la `1.0.1` también: `lifecycle-runtime-compose` 2.11 exigía compilar contra la API 37, y ni siquiera se usaba. Se quitó y la librería sigue en compileSdk 36. Las dos se arreglaron probando el commit en JitPack antes de etiquetar, y como la 1.0.0 nunca llegó a construirse —nadie pudo descargarla—, **se reapuntó el tag `1.0.0` al commit bueno** y se borraron los tags `1.0.1` y `1.0.0`. Hay una sola versión, la `1.0.0`. Las siguientes las publica el workflow `.github/workflows/release.yml` al subir `version` en `main`, y conviene lanzar antes la build del commit en JitPack (`https://jitpack.io/com/github/BonyGoD/AdMobKMP/<commit>/build.log`) para no quemar números.
...
**Requisito:** la fase 6 cerrada — tag `1.0.0` publicado y los dos klibs de iOS
...
- `gradle/libs.versions.toml`: `admob-kmp = "1.0.0"` y
  `bonygod-admobkmp = { module = "com.github.BonyGoD.AdMobKMP:admob-kmp", version.ref = "admob-kmp" }`,
...
- **`project.pbxproj` no se edita a mano.** El cambio de paquete local a remoto lo hace
  ...
  `https://github.com/BonyGoD/AdMobKMP`, versión `1.0.0`, producto `AdMobKMPSwift`.
